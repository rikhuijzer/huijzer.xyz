...
# ╔═╡ 028db4d8-1a5f-11ef-013c-cb1568d2e267
# hideall
let
	using CairoMakie
	using CSV
...
# ╔═╡ d56e98ef-bc0b-42c4-acc4-8e9425633212
md"""
...
From $100 to about $0.3, a factor 333 improvement!

![Solar PV prices versus cumulative capacity](https://ourworldindata.org/images/published/solar-pv-prices-vs-cumulative-capacity_1350.png)
...
I think it's the same for solar panel, chip, or battery production.
...
Legal industry prices by the availability of workers, since it's hard to find or train more lawyers.
Oil prices by the availability of oil.
...
# ╔═╡ 597e20ae-d194-4e2c-84de-1a3f184e44d9
md"""
## Batteries
...
# ╔═╡ aae31cec-c7ba-47f1-bddb-b0f15a28fdcd
# hideall
function y(xs, start_y::AbstractFloat, improvement_rate)
	@assert 1 < improvement_rate < 100
	change = 1.0 - (improvement_rate / 100)
	[start_y * change^i for i in 1:length(xs)]
end;

# ╔═╡ 23434a65-71d5-48c5-9d9e-38bdc3459b9a
# hideall
function theme()
    Theme(
        Axis = (;
...
# ╔═╡ 291ed582-613e-4d37-85ba-84f22ea4a065
# hideall
color = :black;

# ╔═╡ 6e726c13-124c-4a99-b181-dca8912ab522
# hideall
with_theme(theme()) do
	title = "Li-on Battery Cost Projection - \$/kWh"
	subtitle = "(12% cost improvement rate)"
	fig = Figure(; size=(600, 400))

	xs = 2010:2030
	ys = y(xs, 1000.0, 12)
	yticks = [ceil(Int, 1 * 2^i) for i in 1:20]
	# yticks = [yticks; 200]
	yticklabels = ["\$$y" for y in yticks]
	yticks = (yticks, yticklabels)
	
	ax = Axis(fig[1, 1]; title, subtitle, yticks, yscale=log10)
	lines!(ax, xs, ys; color)
	
	fig
end

# ╔═╡ 6c853c3d-b6cc-4f6f-9f00-38f31638c41b
md"""
...
# ╔═╡ e0038596-a727-472c-a45a-136726cd91e2
# hideall
with_theme(theme()) do
	title = "Li-on Battery Cost Projection - \$/kWh"
	subtitle = "(16% cost improvement rate)"
	fig = Figure(; size=(600, 400))
...
# ╔═╡ 75a98ce3-b3a3-4edc-825e-a6eafa3ef6f4
md"""
...
# ╔═╡ a5473369-c059-455d-9bab-2f9ff36ab863
# hideall
bloomberg_data = let
	xs = 2013:2024
	ys = [
...
# ╔═╡ 8cce6a3b-a9c1-4db8-b97a-80d62adc5b38
# hideall
with_theme(theme()) do
	title = "Battery Cost Projection - \$/kWh"
	fig = Figure(; size=(600, 400))
	...
	subtitle = "(12% and 16% cost improvement rate)"
	ax = Axis(fig[1, 1]; title, yticks, subtitle, yscale=log10)
	lines!(ax, xs, ys; color)
	...
	xs, ys = bloomberg_data
	scatter!(ax, xs, ys; color)

	text!(ax, 2028, 90; text="12%")
	text!(ax, 2028, 36; text="16%")

	fig
end

# ╔═╡ b6d469ff-f026-4dc0-8c16-5ea7bfa321bb
md"""
...
# ╔═╡ d0685ab7-6913-4264-b7e2-442a87d4953b
md"""
...
manifest_format = "2.0"
project_hash = "6c2cb313a831321d5baa5bce7e522ebddc597f93"
