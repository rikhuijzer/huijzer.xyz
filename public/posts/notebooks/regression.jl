...
# ╔═╡ c6b9b44f-9d32-4615-a91d-a0cbbbbd7dc7
begin
...
# ╔═╡ bc90c77a-4a1c-11ec-3761-392a0284dc6c
md"""
...
# ╔═╡ 7f20b8f3-e1e7-40ff-9eef-8db03f9daf4f
r_2(x) = round(x; digits=2);

# ╔═╡ 97bce1b6-5916-4ec5-a3c3-16b1e6fafe5e
df = let
 	rng = StableRNG(42)
	n = 12
	I = 1:n
...
# ╔═╡ 36764a36-4109-4dab-9b14-3955501799ec
md"""
...
# ╔═╡ ddae304f-e37d-471d-9406-3491e3de6174
# hideall
begin
	wmin = minimum(df.W) - 0.2
	wmax = maximum(df.W) + 0.2

	fg = data(df) * mapping(:W, :H)
	draw(fg)
end

# ╔═╡ ee2cd7b0-4616-4bd0-9dd2-92faaa51238d
md"""
...
# ╔═╡ 91cf010c-cc00-429a-8c9d-deaac51567ca
# hideall
let
	...
	for (w, h) in zip(df.W, df.H)
	  df_diff = (W=[w, w], H=[m, h])
	  layers += data(df_diff) * visual(Lines)
	end

	draw(layers * mapping(:W, :H))
end

# ╔═╡ a13ec4c0-44b0-4465-9673-cf2632dcc939
md"""
...
# ╔═╡ 754f2cf3-deff-4fa6-b6ab-ea7e5074529a
S(U, V) = sum((U .- V).^2)

# ╔═╡ e261a5f6-02f6-44d1-8f92-ec3f7e05bd48
md"""
Then, the squared sum of the errors for this simplest estimator is
"""

# ╔═╡ 922458a0-b57f-486c-af85-3a510da3b7f4
r_2(S(df.H, repeat([mean(df.H)], length(df.H))))

# ╔═╡ 7500bde5-7c01-4c2d-9a61-9ac81dfe1e21
md"""
...
# ╔═╡ 265ac353-dcfb-478b-9ea6-1f07380790c6
m1 = lm(@formula(H ~ W), df);

# ╔═╡ cba9741c-d610-44be-a1c5-6533a8c7d174
"""
...
# ╔═╡ f550d377-ca4e-4051-913f-d32f64d44b2c
# hideall
let
...
# ╔═╡ 14ac6afa-557e-461c-b2cd-b524fa820990
md"""
The intercept and slope for the fitted line are
"""

# ╔═╡ 1afb0f9d-6d2c-4718-acbf-2721bf1ee336
intercept(linear_model) = coef(linear_model)[1]

# ╔═╡ c81233cb-52fd-44f5-8147-7eac6caa3b3c
slope(linear_model) = coef(linear_model)[2]

# ╔═╡ a1470d05-6b4a-43ca-9253-3f4a9427d789
r_2(intercept(m1))

# ╔═╡ d1f4af79-2ad9-42c9-baca-f534e29d48d9
r_2(slope(m1))

# ╔═╡ c81a4089-14e5-419a-b395-7becc66e9edf
md"""
...
# ╔═╡ 7eefa2cb-e1df-4c35-9a73-09d2165878e9
df_digits = let
...
# ╔═╡ 727613f4-312d-40bb-a06a-3717f31fed3e
m2 = lm(@formula(Y_digit ~ W), df_digits);

# ╔═╡ 6eca62b5-eb97-4694-8c1b-fc5b623b68e1
# hideall
let
	df = df_digits
...
# ╔═╡ 9f3eec01-dc02-4aa9-b83c-3ca6884ecf77
md"""
...
# ╔═╡ c3304471-20b5-4984-8e11-fe1945c66eb2
m3 = glm(@formula(Y_digit ~ W), df_digits, Binomial(), LogitLink());

# ╔═╡ add010a2-5dd1-40e8-933e-bd7720da2249
# hideall
let
	df = df_digits
...
# ╔═╡ cefcbda8-f9b3-4bbb-a4db-d428806a4bc4
md"""
...
# ╔═╡ 3cc7c44c-813f-4f40-83db-5de2a9123080
md"""
$$\text{accuracy} = \frac{\text{number of correct predictions}}{\text{total number of predictions}} .$$
"""

# ╔═╡ a685d2c0-d84b-4351-87a6-46eefd89f442
accuracy(trues, pred) = count(trues .== pred) / length(pred)

# ╔═╡ f6aee25c-9b31-4409-9138-98ba796350cf
md"""
...
# ╔═╡ a5253018-fce3-45b3-94da-8b43c8f747bb
binary_values(model) = [0.5 < x ? 1 : 0 for x in predict(model)]

# ╔═╡ 08657ee7-520a-4435-b32b-1b62c5b7d943
let
...
# ╔═╡ 659a84b8-37ab-425d-bc05-b530b0cd6240
md"""
...
# ╔═╡ 1bd3f499-9bd4-49c9-92ab-47ecda206343
md"""
## References
...
manifest_format = "2.0"
project_hash = "b32312ed1fcf57c424c6fe315272796c4048fc62"
