...
# ╔═╡ 1fc8cb84-4eae-11ec-0cb2-63bbb54ccb4a
begin
...
# ╔═╡ 52285af7-74a4-48f1-9893-53fd12251fbd
md"""
...
# ╔═╡ ddbaf336-9f4a-452e-aac3-2a4f48545582
d1 = Normal(22, 2.4); # Australian Shepherd.

# ╔═╡ 24c0fc5c-a79b-4bf2-9137-05f51a8219a6
d2 = Normal(28, 2.8); # American Terrier.

# ╔═╡ b5f7382c-8ccd-4dc4-893d-52b746a5c9f9
combined = let
	rng = StableRNG(1)
	[i % 2 == 1 ? rand(rng, d1) : rand(rng, d2) for i in 1:200]
end;

# ╔═╡ a5813e90-11e3-4b13-a2a3-814ccf1cf7d7
md"and visualize it:"

# ╔═╡ e1de63cd-8543-4106-a83a-bc3cf4b8e5e6
md"""
...
# ╔═╡ f93afb03-fa34-4cb5-b68b-0cee20ef29ca
md"""
## First attempt
...
# ╔═╡ 16b1ef43-0c71-4130-92f2-5f35720f46ce
...
# ╔═╡ 65ba11c5-be4a-45e7-9126-6cc16cdaf42f
exchangeable_model = exchangeable_mixture(2, combined);

# ╔═╡ bf1339b3-c166-48cb-9fb2-345419754be4
n_samples = 1_000;

# ╔═╡ 755b773c-2ade-40ae-a778-f24b57b38a53
md"""
Just to be sure, let's sample from the prior to see how things look:
"""

# ╔═╡ 25a8d1e1-55f6-4ef4-9b4e-72c8b8cb8362
exchangeable_prior = sample(exchangeable_model, Prior(), n_samples);

# ╔═╡ 9812cc39-b9c7-4fbb-88d0-a9129889d9da
md"""
...
# ╔═╡ 0665f0a1-4b74-4e20-a551-695ca716608b
...
# ╔═╡ 524b74ab-0838-4b31-b403-dec007f43cba
md"""
Hmm.
That didn't work.
When the one or more chains don't move at all (a horizontal line in the left plot) for mixture models, then try reducing the leapfrog step size (the first argument to `HMC`).
"""

# ╔═╡ 58e9d866-658b-4742-83ed-242faccf046d
...
# ╔═╡ a1dd19bd-203b-400b-8c05-729b50c35867
md"""
...
# ╔═╡ e1b3b98d-839f-4f5f-a223-db77421d055a
md"""
...
# ╔═╡ 21742453-221d-4c3e-af00-0731771889eb
...
# ╔═╡ 764ef04e-109c-46b4-81e1-390b1a805cb8
mixture_model = mm(2, combined);

# ╔═╡ c1184d0f-b1ef-4682-8617-40c1c52e5772
mixture_model_prior = sample(mixture_model, Prior(), n_samples);

# ╔═╡ 5708b045-f57b-485f-b7d1-34dede0345f1
md"""
...
# ╔═╡ c63b52e6-9fcf-4a18-bfab-7979814d8cd6
...
# ╔═╡ 54ad3b39-a2bd-45e2-9196-c4d2fd06eb3f
# hideall
describe(mixture_posterior)[1]

# ╔═╡ b6442aa1-de95-4698-9153-775f37630125
md"""
And we now have almost correct estimates for all parameter locations, see the `mean` column.

To make this even better, it is possible to tune the `HMC` parameters based on how well the chains converge or by setting a slightly more informative prior.
In this case, by setting two non-overlapping truncated normals:
"""

# ╔═╡ 582596d7-e517-48ec-801e-ca4b0d7eac54
...
# ╔═╡ 405ea298-272e-413d-a072-d0216ed53571
normal_mixture_model = normals_mm(2, combined);

# ╔═╡ 45156784-5b96-4747-95a5-fb64bee7101d
...
# ╔═╡ 503fa826-d396-42bc-9d8b-31e7fb02cfa1
# hideall
describe(normal_mixture_posterior)[1]

# ╔═╡ 10a413c7-018d-4b5a-8b48-f25e95fe04b8
md"""
Finally, this seems to properly converge.
Alas, the model is now quite sensitive to choice of prior and choice of sampler parameters.
"""

# ╔═╡ e70dcf9e-08cb-40c4-a633-ea7627c23d8b
md"""
...
# ╔═╡ 9f5da6b7-c529-4181-8f4e-58affe8e03af
...
# ╔═╡ 29566145-9151-447d-bb98-cdcd75087d81
# ╠═╡ show_logs = false
vi_posterior = sample_vi(mixture_model);

# ╔═╡ e5a49ca9-e30b-4e03-9584-3d6e0715630b
# hideall
mean_std(V) = [f(V) for f in [mean, std]];

# ╔═╡ 3a68120e-1c3b-444f-a977-03262abb8191
# hideall
function vi_mean_std(param)
	v = mean_std(vi_posterior[:, param])
	v = round.(v; digits=3)
	return Base.Text("$param: $v")
end;

# ╔═╡ bc999d8a-4d94-478e-8de4-2412dc3d3c3f
# hideall
let
	parameters = names(mixture_model_prior, :parameters)
	M = [mean(vi_posterior[:, p]) for p in parameters]
	S = [std(vi_posterior[:, p]) for p in parameters]
	df = DataFrame(; parameters, M, S)
	select(df, :parameters, :M => :mean, :S => :std)
end

# ╔═╡ f52299a9-8b5c-41d7-906b-3e6980a1faf6
md"""
This outcome is
...
# ╔═╡ 3e24f78f-15c9-436c-9611-acc81177db26
md"""
## Ordered constraint
...
# ╔═╡ 2b94ceb7-be3b-49dd-83f2-690f0607d5b1
inv_ordered(X::Vector) = Bijectors.inverse(Bijectors.OrderedBijector())(X);

# ╔═╡ 87903a4a-4359-4192-9fc5-2c1c9d8435a5
M = inv_ordered([25, 25.01])

# ╔═╡ b9be7de6-e2cd-4fb3-b157-4857ab5c2972
...
# ╔═╡ 757aa80b-bf35-4424-980a-e273bee9b9dd
ordered_model = ordered_mixture(2, combined);

# ╔═╡ c0e04959-5253-45d2-b78e-1375674fa952
md"""
In the end, the HMC sampler keeps being the most robust.
I've tried `NUTS(1_000, 20)` too and it did work albeit taking minutes to finish and giving erratic w estimates.
...
# ╔═╡ 1d7c0541-afc9-4053-9d96-f8c99276cd14
...
# ╔═╡ bd7d0155-473d-425a-b7b8-28d99d674712
md"""
One last thing.
Convergence takes a while, so let's throw away the first 700 samples.
"""

# ╔═╡ 60a5e6cc-3918-4641-9b86-eb765c0fc6c4
...
# ╔═╡ 2499bd65-b16f-42d5-9b3f-b4180b8696f5
# hideall
describe(ordered_warmed_posterior)[1]

# ╔═╡ 4573118d-3135-4ab1-80c0-2b359cb2d883
md"""
Awesome.
"""

# ╔═╡ 04c5e772-4a1c-4309-a26d-b4f722984aa2
md"""
## Conclusion
...
# ╔═╡ 8e5e8063-ba6a-4c9a-bc16-7b1f2338644b
md"""
## References
...
# ╔═╡ ffcd2fcc-79d9-4118-8323-849df7cf21d0
# hideall
...
# ╔═╡ a8e9c4e8-3493-49ab-81a7-9edcbca06524
# hideall
plot_latent(d1, d2, combined)

# ╔═╡ d7d91ea5-c745-442f-a51c-b1c0bc710a0b
# hideall
...
# ╔═╡ b9365ded-2d96-426c-bbb0-ab947c5ac321
# hideall
plot_chains(exchangeable_prior; density_func=hist!)

# ╔═╡ 5c9b3d2f-35c4-4d2e-8edc-6f59fc97f38a
# hideall
plot_chains(exchangable_posterior)

# ╔═╡ 2c286291-e445-42c8-96fc-518eff678306
# hideall
plot_chains(exchangable_posterior_smaller_stepsize)

# ╔═╡ 9a124579-abf0-4344-ab1c-c0f3f132ab97
# hideall
plot_chains(mixture_model_prior; density_func=hist!)

# ╔═╡ 317767c4-cf95-4722-82be-bcd9ccd53b11
# hideall
plot_chains(mixture_posterior)

# ╔═╡ 99e02e0b-4df5-4a10-9b99-98191800c04d
# hideall
plot_chains(normal_mixture_posterior)

# ╔═╡ ed66902d-31bc-4780-9b8b-bfe2441df8e3
# hideall
plot_chains(vi_posterior)

# ╔═╡ b27ed408-25a6-4d1f-bb2f-40e8b6788c96
# hideall
plot_chains(ordered_hmc_posterior)

# ╔═╡ 821ffaea-993f-4499-9644-8be1553b1215
# hideall
plot_chains(ordered_warmed_posterior)
...
manifest_format = "2.0"
project_hash = "4e56cbc39401e5e3b8b863a5c94a2608021d0b2e"
...
git-tree-sha1 = "93991bc49e16632794fa3b8a8836ccce24959264"
...
git-tree-sha1 = "6ff628d549699f28c507d1a9289e05c8756fa39c"
...
git-tree-sha1 = "af48352d01f1654df2d83668dd6f3dc445065da4"
