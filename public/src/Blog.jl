...
const PROJECT_ROOT = string(pkgdir(Blog))::String
...
function build(files::Union{Vector,Nothing}=nothing)
    dir = joinpath(PROJECT_ROOT, "posts", "notebooks")
    ...
    hopts = OutputOptions(; append_build_context)
    if isnothing(files)
